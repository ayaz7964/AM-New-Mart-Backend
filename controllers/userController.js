...
exports.registerUser = asyncErrorHandler(async (req, res, next) => {
    try {
        ...
        // If avatar is sent as a file (multipart/form-data)
        if (req.files && req.files.avatar) {
            // Convert file buffer to base64
            const file = req.files.avatar;
            const base64 = file.data.toString('base64');
            // Use the mimetype from the uploaded file
            avatar = `data:${file.mimetype};base64,${base64}`;
        }
        ...
        // Validate and format avatar as base64 data URI if needed
        let avatarData = avatar;
        if (!/^data:image\/[a-zA-Z]+;base64,/.test(avatar)) {
            // If avatar is just base64, prepend data URI header (assuming PNG)
